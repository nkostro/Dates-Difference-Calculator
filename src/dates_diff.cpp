...
int main() // <- точка входа в программу
{
    ...
    // {} - списковая инициализация - забивает массив дефолтными значениями;
    // в данном случае - нулями.
    int years[ndates] = {};

    // В цикле просто вычисляю число дней с начала года до каждой из дат, включительно.
    for (int i = 0; i < ndates; ++i) { // повторяем для каждой даты
        ...
        if (!is_valid_date(day, month, year))
            return 1; // просто взял и вышел, если дата некорректна
        ...
        // не работает и нужно добавить +1 месяц (во всем виноват август)
